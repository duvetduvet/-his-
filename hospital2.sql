...
INSERT INTO `medicine` VALUES ('640', '', '非医保', '啊啊啊', '', '1', '0.00', '6.00', '2', null, null, null, 'AAA', '诊疗费', '0', '1');
...
CREATE TABLE `order` (
  ...
  `medicinediscount` decimal(5,2) NOT NULL DEFAULT '1.00',
  `medcurediscount` decimal(5,2) NOT NULL DEFAULT '1.00',
...
-- Records of order
-- ----------------------------
INSERT INTO `order` VALUES ('1', '1', null, null, null, '15.00', '5.00', '0.00', '1.00', '1.00', '2018-12-24 15:48:55', '20.00', null);
INSERT INTO `order` VALUES ('3', '1', null, null, null, '0.00', '0.00', '10.00', '1.00', '1.00', '2018-12-25 15:19:37', '10.00', '0124116072');
INSERT INTO `order` VALUES ('5', '1', null, null, null, '0.00', '0.00', '16.00', '1.00', '1.00', '2018-12-25 15:26:45', '16.00', '0124118964');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `count_depart_view` AS select `temp`.`name` AS `name`,`temp`.`income` AS `sumincome`,`temp`.`feetype` AS `feetype`,`temp`.`date` AS `date` from (select `re`.`departname` AS `name`,`re`.`fee` AS `income`,'挂号费' AS `medname`,'挂号费' AS `feetype`,`re`.`date` AS `date` from `hospital2`.`all_register_view` `re` union select `po`.`departname` AS `name`,((`pre`.`outprice` * `pre`.`number`) * `po`.`medicinediscount`) AS `income`,`pre`.`name` AS `medname`,`pre`.`feetypename` AS `feetype`,`po`.`date` AS `date` from (`hospital2`.`prescription_view` `pre` join `hospital2`.`pre_order_view` `po`) where ((`po`.`pid` = `pre`.`prescriptionid`) and (`po`.`state` >= 2) and ((`po`.`card` = 0) or isnull(`po`.`card`)) and ((`po`.`given` = 0) or isnull(`po`.`card`)))) `temp` ;

-- ----------------------------
-- View structure for count_doctor_view
-- ----------------------------
DROP VIEW IF EXISTS `count_doctor_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `count_doctor_view` AS select `temp`.`name` AS `name`,`temp`.`income` AS `sumincome`,`temp`.`feetype` AS `feetype`,`temp`.`date` AS `date` from (select `re`.`doctorname` AS `name`,`re`.`fee` AS `income`,'挂号费' AS `medname`,'挂号费' AS `feetype`,`re`.`date` AS `date` from `hospital2`.`all_register_view` `re` union select `po`.`doctorname` AS `name`,((`pre`.`outprice` * `pre`.`number`) * `po`.`medicinediscount`) AS `income`,`pre`.`name` AS `medname`,`pre`.`feetypename` AS `feetype`,`po`.`date` AS `date` from (`hospital2`.`prescription_view` `pre` join `hospital2`.`pre_order_view` `po`) where ((`po`.`pid` = `pre`.`prescriptionid`) and (`po`.`state` >= 2) and ((`po`.`card` = 0) or isnull(`po`.`card`)) and ((`po`.`given` = 0) or isnull(`po`.`card`)))) `temp` ;
